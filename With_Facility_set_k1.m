...
for y=1:100 
    rescueNode1=N1index(ceil(rand()*size(N1index,2)));  
    rescue_pathlength=zeros(num,size(rescueNode1,2));
    rescue_path=cell(num,size(rescueNode1,2));
    for i=1:num
        for j=1:size(rescueNode1,2)
            [rescue_pathlength(i,j) rescue_path{i,j}]=graphshortestpath(sparse(matrix_length),rescueNode1(j),node(i));
        end
    end
