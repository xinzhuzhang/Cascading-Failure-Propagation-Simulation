...
for y=1:100 
    rescueNode3=N3index(ceil(rand()*size(N3index,2)));
    rescue_pathlength=zeros(num,size(rescueNode3,2));
    rescue_path=cell(num,size(rescueNode3,2));
    for i=1:num
        for j=1:size(rescueNode3,2)
            [rescue_pathlength(i,j) rescue_path{i,j}]=graphshortestpath(sparse(matrix_length),rescueNode3(j),node(i));
        end
    end
