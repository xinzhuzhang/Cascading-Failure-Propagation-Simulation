...
for y=1:100 
    rescueNode2=N2index(ceil(rand()*size(N2index,2)));
    rescue_pathlength=zeros(num,size(rescueNode2,2));
    rescue_path=cell(num,size(rescueNode2,2));
    for i=1:num
        for j=1:size(rescueNode2,2)
            [rescue_pathlength(i,j) rescue_path{i,j}]=graphshortestpath(sparse(matrix_length),rescueNode2(j),node(i));
        end
    end
