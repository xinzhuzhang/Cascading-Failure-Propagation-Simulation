...
for y=1:100 
    rescueNode5=N5index(ceil(rand()*size(N5index,2)));
    rescue_pathlength=zeros(num,size(rescueNode5,2));
    rescue_path=cell(num,size(rescueNode5,2));
    for i=1:num
        for j=1:size(rescueNode5,2)
            [rescue_pathlength(i,j) rescue_path{i,j}]=graphshortestpath(sparse(matrix_length),rescueNode5(j),node(i));
        end
    end
