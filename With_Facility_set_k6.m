...
for y=1:100 
    rescueNode6=N6index(ceil(rand()*size(N6index,2)));
    rescue_pathlength=zeros(num,size(rescueNode6,2));
    rescue_path=cell(num,size(rescueNode6,2));
    for i=1:num
        for j=1:size(rescueNode6,2)
            [rescue_pathlength(i,j) rescue_path{i,j}]=graphshortestpath(sparse(matrix_length),rescueNode6(j),node(i));
        end
    end
